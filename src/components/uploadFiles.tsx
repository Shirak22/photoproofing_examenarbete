...
export default function UploadFiles({ albumId }: { albumId: string }) {
  ...
  return (
    <div className=" flex max-w-md ml-auto mb-4 relative  "  onClick={handleClickOutside}>
      <button className=" bg-slate-500 text-white p-1 text-xs px-2 rounded-md ml-auto" onClick={() => setOpen(!open)}>Add images</button>
      {open && (
        <form
      ...
      >
        ...
        <UploadFormWrapper>
          ...
          <div className="flex mt-4">
            <p className="text-xs mt-auto text-slate-500"> {res?.message} </p>
